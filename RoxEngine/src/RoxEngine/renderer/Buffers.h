...
namespace RoxEngine {
	...
	public:
		...
		inline static std::shared_ptr<VertexBuffer> Create(const BufferLayout& layout, size_t size) {
			return Create(layout, nullptr, size);
		}
	...
	{
	public:
		static std::shared_ptr<IndexBuffer> Create(uint32_t* data, size_t count);
		inline static std::shared_ptr<IndexBuffer> Create(size_t count) {
