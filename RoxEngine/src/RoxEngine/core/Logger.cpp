...
namespace RoxEngine {
	...
	void Logger::Init()
	{
		...
		logSinks.emplace_back(std::make_shared<spdlog::sinks::basic_file_sink_mt>("RoxEngine.log", true));

		logSinks[0]->set_pattern("%^[%T] %n: %v%$");
		logSinks[1]->set_pattern("[%T] [%l] %n: %v");

		sCoreLogger = std::make_shared<spdlog::logger>("ROXENGINE", begin(logSinks), end(logSinks));
